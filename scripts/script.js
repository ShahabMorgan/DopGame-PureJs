document.querySelector("#games-button-start").addEventListener("click", () => {
  startTimer();
});
...
elmentLentgh = 0;
dopTextVisiblity(localStorage.getItem("dopTextVisiblity"));
...
let permistionToStart =
  JSON.parse(localStorage.getItem("permistionToStart")) || false;

let displayInStart =
  JSON.parse(localStorage.getItem("displayInStart")) || false;
...
function startTimer() {
  ...
  timer = setTimeout(() => {
    ...
    localStorage.setItem(
      "permistionToStart",
      JSON.stringify(permistionToStart)
    );
  ...
  localStorage.setItem("dopTextVisiblity", "none");
}
...
function gamePoint() {
  if (displayInStart === false) {
    score.point1 = Number(
      document.getElementsByClassName("is-lose").length * -1
    );
    score.point2 = Number(document.getElementsByClassName("is-win").length);
  }

  displayPoint();
  localStorage.setItem("dopTextVisiblity", "block");
...
function displayPoint() {
  document.querySelector("#js-p-1").innerText = `Your point is = ${
    score.point1 + score.point2
  }
    Level = ${pointTo}
    `;
}
...
function dopTextVisiblity(visiblity) {
  document.querySelector("#dop-game-text-js").style.display = visiblity;
}
